...
public class ErrorsMerger {
	 ...
	 * @param newErrors
	 *            Additional errors collection
	 * @param relativePath
	 *            Path of newErrors object in errors object
	 */
	public static void mergeErrors(Errors errors, Errors newErrors, String relativePath) {
		...
		for (FieldError e : newErrors.getFieldErrors()) {
			errors.rejectValue(	StringFunc.concatWithDelim(relativePath, DELIMITER, e.getField()),
								e.getCode(),
								e.getArguments(),
								e.getDefaultMessage());
		}
	...
	public static String asString(Errors errors) {
		...
		if (errors != null) {
			for (ObjectError e : errors.getGlobalErrors()) {
				result.append("error object " + e.getObjectName() + ": " + e.getCode() + "("
					+ e.getArguments() + ") // " + e.getDefaultMessage());
			}
			for (FieldError e : errors.getFieldErrors()) {
				result.append("error field " + e.getField() + ": " + e.getCode() + "("
					+ e.getArguments() + ") // " + e.getDefaultMessage());
			}
		}
